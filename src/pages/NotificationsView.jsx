...
import { 
  collection, query, where, orderBy, limit, onSnapshot, doc, updateDoc, writeBatch, addDoc 
} from "firebase/firestore";
import { db } from "../firebaseConfig";
import { Bell, Trash2, CheckCircle, Zap, X, MessageSquare, Loader2, Check, CornerUpLeft, AlertTriangle } from "lucide-react";
...
export default function NotificationsView({ currentUser }) {
  ...
  // --- NOVO: ESTADO PARA O MODAL DE LIMPEZA ---
  const [isClearModalOpen, setIsClearModalOpen] = useState(false);
  const [isClearing, setIsClearing] = useState(false);

  const quickReplies = ["Recebido! ⚡", "Tmj! 👊", "Devolvendo a energia! 🔋", "Opa! 👀", "Valeu! ❤️"];
    ...
    <div className="p-4 space-y-4 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <div className="flex justify-between items-center px-2 mb-2">
        ...
        {notifications.length > 0 && (
          <button 
            // AQUI MUDOU: Em vez de window.confirm, abrimos nosso modal
      ...
      <div className="space-y-3">
        {notifications.map((notif) => (
          <div 
            ...
            `}
          >
            {!notif.read && <span className="absolute top-4 right-4 w-2 h-2 bg-indigo-500 rounded-full animate-pulse shadow-sm shadow-indigo-300"></span>}

            <div className="flex items-start gap-3">
              <div className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${notif.type === 'energy' ? 'bg-yellow-100 text-yellow-600' : 'bg-slate-100 text-slate-500'}`}>
                {notif.type === 'energy' ? <Zap size={20} className="fill-current"/> : <Bell size={20}/>}
              </div>

              <div className="flex-1">
                <p className="font-bold text-slate-800 text-sm mb-0.5">{notif.fromName || "Alguém"}</p>
                <p className="text-sm text-slate-600 leading-snug">{notif.message}</p>
                
                <div className="flex items-center justify-between mt-2">
                    <p className="text-[10px] text-slate-400 font-medium">
                    {notif.timestamp?.toDate ? notif.timestamp.toDate().toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'}) : "Agora"}
                    </p>
                    {notif.type === 'energy' && (
      ...
      {/* --- MODAL 1: CONFIRMAÇÃO DE LIMPEZA (NOVO) --- */}
      {isClearModalOpen && (
          <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
             <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200 text-center">
                ...
                <h3 className="font-bold text-lg text-slate-800 mb-2">Limpar tudo?</h3>
                <p className="text-sm text-slate-500 mb-6">Você vai apagar todas as notificações recebidas. Essa ação não pode ser desfeita.</p>
                ...
                <div className="flex gap-3">
                    ...
                    >
                        {isClearing ? <Loader2 size={16} className="animate-spin"/> : "Apagar"}
                    </button>
                </div>
             </div>
          </div>
            ...
            <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-300">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                        Responder <span className="text-indigo-600">{replyTarget.name.split(" ")[0]}</span>
                    </h3>
                    <button onClick={() => setIsReplyModalOpen(false)} className="bg-slate-100 p-2 rounded-full text-slate-400 hover:bg-slate-200">
                        <X size={18} />
                    </button>
                </div>
                    ...
                    <>
                        ...
                        <div className="flex flex-wrap gap-2 mb-6">
                            {quickReplies.map((msg) => (
                                <button key={msg} onClick={() => setMessageText(msg)} className="text-xs bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg hover:bg-indigo-100 hover:text-indigo-600 transition-colors border border-slate-200">{msg}</button>
                            ))}
                        </div>
                        <button onClick={handleSendReply} disabled={sendingState === "sending"} className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold text-lg shadow-lg hover:bg-indigo-700 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-70">
                            {sendingState === "sending" ? <Loader2 className="animate-spin" /> : <><Zap className="fill-current" /> Enviar</>}
                        </button>
                    </>
                )}
            </div>
