...
import { 
  ...
  MessageSquare, AlertCircle, Bell, Battery, Trash2, Clock, AlertTriangle 
} from "lucide-react";
...
export default function FriendsView({ currentUser }) {
  ...
  // Modais
  const [modalState, setModalState] = useState({ type: null, data: null }); // type: 'message' | 'remove'
  const [messageText, setMessageText] = useState("");
  const [actionStatus, setActionStatus] = useState("idle"); // 'idle', 'loading', 'success'
  ...
  // --- 1. CARREGAMENTO DE DADOS (REAL-TIME & POLLING) ---
  useEffect(() => {
    if (!currentUser?.uid) return;

    // A. Escuta Solicitações de Amizade (Tempo Real)
    ...
    // B. Carrega Lista de Amigos e Baterias (Polling Inteligente)
    const loadFriends = async () => {
        if (friends.length === 0) setLoading(true); // Loading só na 1ª vez
        
        try {
          // Busca conexões aceitas (Eu -> Eles e Eles -> Eu)
          ...
          // Guardamos o ID do Request para poder deletar a amizade depois
          snap1.forEach(d => friendMap.set(d.data().from, d.id)); 
          snap2.forEach(d => friendMap.set(d.data().to, d.id));   
          ...
          // Ordena: Menor bateria primeiro (quem precisa de ajuda no topo)
          setFriends(friendsList.sort((a, b) => a.finalBattery - b.finalBattery));
        } catch (e) { console.error(e); } finally { setLoading(false); }
    };
    
    loadFriends();
    const interval = setInterval(loadFriends, 5000); // Atualiza bateria a cada 5s
  ...
  // Buscar Usuários (Lógica Universal)
  ...
  return (
    <div className="p-4 space-y-4 pb-24 animate-in fade-in duration-500 min-h-screen bg-slate-50 relative">
      ...
      <div className="flex bg-white p-1 rounded-2xl shadow-sm mb-4 sticky top-0 z-30">
        ...
        <button onClick={() => setView('requests')} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2 relative ${view === 'requests' ? 'bg-indigo-100 text-indigo-700' : 'text-slate-400'}`}>
            <UserPlus size={18}/>
            {requests.length > 0 && <span className="absolute top-1 right-3 bg-red-500 w-2.5 h-2.5 rounded-full animate-ping"></span>}
        </button>
      </div>
            ...
            {loading && friends.length === 0 ? <div className="flex justify-center p-8"><Loader2 className="animate-spin text-indigo-500"/></div> : 
             friends.length === 0 ? (
                <div className="text-center py-10 opacity-60">
                    ...
                    <p className="text-slate-600 font-medium">Sua lista está vazia.</p>
                    <button onClick={() => setView('search')} className="text-indigo-600 font-bold mt-2 underline">Adicionar amigos</button>
                </div>
             ) : (
                friends.map(friend => (
                    <div key={friend.uid} className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between transition-all active:scale-[0.98]">
                    ...
                    <div key={user.uid} className="bg-white p-4 rounded-2xl shadow-sm flex justify-between items-center animate-in slide-in-from-bottom-2">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md flex items-center gap-1 w-fit mt-1"><Hash size={10} /> {user.userTag || "----"}</span>
                            </div>
                        </div>
                        <button onClick={() => sendFriendRequest(user)} className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-indigo-100 transition-colors">Convidar</button>
                    </div>
                ))}
                {searchResults.length === 0 && searchTerm && !loading && <div className="text-center py-8 opacity-60"><p>Ninguém encontrado.</p><p className="text-xs mt-1">Tente buscar pelo email exato.</p></div>}
            </div>
        </div>
      )}
                ...
                <div key={req.requestId} className="bg-white p-4 rounded-2xl shadow-sm border border-indigo-100 flex justify-between items-center animate-in slide-in-from-left-2">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span className="text-xs text-slate-400">quer ser seu amigo</span>
                        </div>
                    </div>
                    <button onClick={() => acceptRequest(req)} className="w-10 h-10 bg-green-100 text-green-600 rounded-full flex items-center justify-center hover:bg-green-200 transition-colors shadow-sm"><Check size={20} /></button>
                </div>
            ))}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                {actionStatus === "success" ? (
                    <div className="text-center py-8"><div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce"><Check size={32} /></div><p className="font-bold text-green-600">Enviado!</p></div>
                ) : (
                    <>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
             <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200 text-center">
                ...
                <div className="flex gap-3">
                    <button onClick={closeModal} className="flex-1 py-3 font-bold text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">Cancelar</button>
                    <button onClick={handleRemoveFriend} disabled={actionStatus === "loading"} className="flex-1 py-3 font-bold text-white bg-red-500 rounded-xl hover:bg-red-600 transition-colors flex items-center justify-center gap-2">{actionStatus === "loading" ? <Loader2 size={16} className="animate-spin"/> : "Remover"}</button>
                </div>
             </div>
          </div>
      )}
