...
export default function HomePage({
  ...
  // 👇 CORREÇÃO AQUI: Passamos 'setBatteryLevel' para o hook!
  const { saveStatus } = useBatterySync(
    userWithGhost,
    batteryLevel,
    setBatteryLevel,
  );
  ...
  return (
    <div
      className={`h-full overflow-hidden font-sans flex flex-col relative w-full transition-colors duration-700 ${isGhostMode ? "bg-slate-200" : "bg-slate-50"}`}
    >
      <div className="flex-1 overflow-y-auto pb-20">
        {activeTab === "home" && (
          <div className="space-y-6 animate-in fade-in duration-500 pt-4 px-2">
            ...
            <div
              className={`rounded-3xl p-6 shadow-xl text-white transition-all duration-700 relative overflow-hidden ${isGhostMode ? "bg-slate-400 grayscale" : `bg-gradient-to-br ${currentMode.accent}`}`}
            >
              {isGhostMode && (
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-4">
                {CONTEXT_TAGS.map((tag) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all active:scale-95 flex items-center gap-1.5 ${isGhostMode ? "opacity-50 cursor-not-allowed border-slate-200 bg-slate-100" : ""} ${selectedTag === tag.id && !isGhostMode ? "bg-indigo-600 text-white border-indigo-600 shadow-md scale-105" : "bg-white text-slate-600 border-slate-200 hover:border-indigo-300"}`}
                  >
            ...
            <div className="relative mx-2">
              ...
              <input
                ...
                disabled={isGhostMode}
                className={`w-full pl-12 pr-4 py-4 rounded-2xl shadow-sm border text-slate-700 placeholder:text-slate-400 focus:outline-indigo-500 transition-all text-sm font-medium ${isGhostMode ? "bg-slate-100 border-slate-200 opacity-60 cursor-not-allowed" : "bg-white border-slate-100"}`}
              />
